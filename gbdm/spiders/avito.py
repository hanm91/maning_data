...
class AvitoSpider(scrapy.Spider):
    ...
    def parse_1(self, response):
        item =  {
            'title': response.xpath(self.__xpath_query['title']).extract_first(),
            'url': response.url,
            'photo': response.xpath(self.__xpath_query['image']).extract(),
            'address': response.xpath(self.__xpath_query['address']).extract_first()
        }
